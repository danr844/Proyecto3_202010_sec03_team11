...
public class Maps extends MapView {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -4308002139824545284L;
	...
	public Maps(GrafoNoDirigido<Integer, GeographicPoint> grafo, RedBlackBST<Integer, Comparendo> comparendos, int p)
	{	
		setOnMapReadyHandler( new MapReadyHandler() {
				@Override
				public void onMapReady(MapStatus status)
				{
			         if ( status == MapStatus.MAP_STATUS_OK )
			         {
			        	...
			        	// Configuracion de localizaciones del path (circulos)
			        	 ...
			        	 if(comparendos!=null)
			        	 {
			        	     Iterator<Comparendo> es = comparendos.keysValue().iterator();
				        	 
				        	 while(es.hasNext())
				        	 {
				        		 Comparendo e = es.next();
